...
abstract public class BaseSynthesizer extends BaseEngine implements Synthesizer {
    ...
    protected void postSpeakableEvent(final SpeakableEvent event, final SpeakableListener extraSpeakableListener) {
     ...
     * listeners.
     */
    public void fireSpeakableEvent(SpeakableEvent event, SpeakableListener extraSpeakableListener) {
        Enumeration E;
        ...
        private void playItens(){
            final int BUFFER_LENGTH = 1024;
             ...
             while(!done){
                 ...
                 postSpeakableEvent(new SpeakableEvent(item.getSource(), SpeakableEvent.TOP_OF_QUEUE,
                 ...
                 while (testEngineState(PAUSED)) {
                     ...
                     try {
                         ...
                         postSpeakableEvent(new SpeakableEvent(item.getSource(),
                                 SpeakableEvent.
                                 SPEAKABLE_RESUMED, item.getId()), item.getListener());
                     ...
                     while ((bytesRead = item.getAudioSegment().getInputStream().read(
                         ...
                         synchronized (cancelFirstItem) {
                             if (cancelFirstItem == true) {
                                 postSpeakableEvent(new SpeakableEvent(item.getSource(),
                                         SpeakableEvent.
                                         SPEAKABLE_CANCELLED, item.getId()), item.getListener());
                                 break;
                             }
                         }
                         ...
                         while (testEngineState(PAUSED)) {
                             postSpeakableEvent(new SpeakableEvent(item.getSource(),
                                 SpeakableEvent.
                                 SPEAKABLE_PAUSED, item.getId()), item.getListener());
                             try {
                                 waitEngineState(BaseEngine.RESUMED);
                                 postSpeakableEvent(new SpeakableEvent(item.getSource(),
                                                                       SpeakableEvent.
                         ...
                         while (wordIndex<item.getWords().length && item.getWordsStartTime()[wordIndex] * sampleRate <= playIndex*bytesRead) {
                             postSpeakableEvent(new SpeakableEvent(item.getSource(),
                                                                   ...
                                                                   wordStart+item.getWords()[wordIndex].length()), item.getListener());
                         ...
                         while (phonemeIndex<item.getPhonesInfo().length && timeNextPhone*sampleRate<playIndex*bytesRead){
                             postSpeakableEvent(new SpeakableEvent(item.getSource(),
                                                                   ...
                                                                   item.getWords()[wordIndex-1], item.getPhonesInfo(), phonemeIndex), item.getListener());
        ...
        public void cancelAllItems() {
            ...
            if (playQueue.size() > 0) {
                ...
                while (playQueue.size()>0){
                    QueueItem item = playQueue.get(0);
                    postSpeakableEvent(new SpeakableEvent(item.getSource(),SpeakableEvent.SPEAKABLE_CANCELLED, item.getId()), item.getListener());
                    playQueue.remove(0);
                }
            }
